...
procedure TA31STAMOV.v_alla_dataExit(Sender: TObject);
begin
  inherited;
  alla_data_controllo;
  if controllo then
...
procedure TA31STAMOV.alla_data_controllo;
begin
  inherited;
  if controllo then
  begin
...
procedure TA31STAMOV.v_tipo_valorizzazioneExit(Sender: TObject);
begin
  inherited;
  //
end;

procedure TA31STAMOV.v_esistenza_zeroExit(Sender: TObject);
begin
  inherited;
  //
end;

procedure TA31STAMOV.controllo_campi;
begin
  alla_data_controllo;
end;

procedure TA31STAMOV.FormShow(Sender: TObject);
var
  tag: tmyquery_go;
begin
  inherited;
...
begin
  ...
  query_qta.sql.add('select ' + quotedstr(utente) + ' utn_codice, art.codice art_codice, art.codice_alternativo art_codice_alternativo,');
  query_qta.sql.add('art.tum_codice art_tum_codice, tma.codice tma_codice, tma.descrizione tma_descrizione,');
  ...
  query_qta.sql.add('F_VALORE_PERS(ART.CODICE, '''',:dalla_data,:alla_data) VALORE_PREC,');
  ...
  query_qta.sql.add('and ( ( F_ESISTENZA_PERS(ART.CODICE, '''',:data_inizio,:data_fine) <> 0 ) or ');
  query_qta.sql.add('  ( ( F_ESISTENZA_PERS(ART.CODICE, '''',:dalla_data,:alla_data) <> 0 )  ) ');
  ...
  try
    query_qta.parambyname('data_fine').asdate := data_fine;
  except
  end;
...
function TA31STAMOV.assegna_costo(tipo_valorizzazione: integer): double;
var
...
procedure TA31STAMOV.v_confermaClick(Sender: TObject);
var
  prosegui: boolean;
  mmt, lif: tmyquery_go;
begin
...
procedure TA31STAMOV.esegue_stampa;
begin
  inherited;
end;

procedure TA31STAMOV.FormCreate(Sender: TObject);
begin
...
procedure TA31STAMOV.call_selart;
begin
...
procedure TA31STAMOV.call_seltma;
begin
...
procedure TA31STAMOV.v_selartClick(Sender: TObject);
begin
  inherited;
  call_selart;
end;

procedure TA31STAMOV.v_seltmaClick(Sender: TObject);
begin
  inherited;
  call_seltma;
end;

procedure TA31STAMOV.v_solo_quantitaClick(Sender: TObject);
begin
  inherited;
  if v_solo_quantita.checked then
  begin
...
procedure TA31STAMOV.ppHeaderBand1BeforePrint(Sender: TObject);
begin
  inherited;
  try
...
procedure TA31STAMOV.pp_DBText2DrawCommandClick(Sender,
...
procedure TA31STAMOV.pp_DBText2DrawCommandCreate(Sender,
  aDrawCommand: TObject);
begin
  inherited;
...
procedure TA31STAMOV.ppReportBeforePrint(Sender: TObject);
begin
  inherited;
  try
...
procedure TA31STAMOV.FormDestroy(Sender: TObject);
begin
  inherited;
