...
type
  TA31STARMA = class(TSTAMPA)
    ...
    ppLine4: TppLine;
    RzLabel1: TRzLabel;
    v_data_consegna_dal: trzdatetimeedit_go;
    ppGroup1: TppGroup;
    ...
    procedure v_data_consegna_dalExit(Sender: TObject);
...
procedure TA31STARMA.FormShow(Sender: TObject);
begin
  inherited;
  promau_inizializza_variabili;

  v_data_consegna_dal.date := now;
end;

procedure TA31STARMA.stampa;
begin
  query.close;
  query.sql.clear;
  ...
  query.parambyname('dal').asdatetime := v_data_consegna_dal.date;
  ...
  query_righe.parambyname('dal').asdatetime := v_data_consegna_dal.date;
...
procedure TA31STARMA.v_confermaClick(Sender: TObject);
begin
  inherited;
  esegui_stampa;
end;

procedure TA31STARMA.v_data_consegna_alEnter(Sender: TObject);
begin
  inherited;
  v_data_consegna_al.date := v_data_consegna_dal.date;
end;

procedure TA31STARMA.v_data_consegna_alExit(Sender: TObject);
begin
  inherited;
  if v_data_consegna_dal.date = 0 then
  begin
    v_data_consegna_al.date := strtodate('29999_12_31');
  end;

  if v_data_consegna_al.date < v_data_consegna_dal.date then
  begin
    messaggio(000, 'Data consegna dal  obbligatoria');
    v_data_consegna_dal.selectall;
    v_data_consegna_dal.setfocus;
    abort;
  end;

end;

procedure TA31STARMA.v_data_consegna_dalExit(Sender: TObject);
begin
  inherited;
  if v_data_consegna_dal.date = 0 then
  begin
