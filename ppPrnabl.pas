...
type
  ...
  protected
    ...
    procedure BorderOnChangeEvent(Sender: TObject); virtual;
    ...
    function  GetmmHeight: Longint; virtual;
    function  GetmmLeft: Longint; virtual;
    function  GetmmTop: Longint; virtual;
    function  GetmmWidth: Longint; virtual;
    function  GetScreenPixelLeft: Integer; virtual;
    function  GetScreenPixelTop: Integer; virtual;
    function  GetScreenPixelWidth: Integer; virtual;
    function  GetScreenPixelHeight: Integer; virtual;
    function  GetTop: Single; virtual;
    function  GetWidth: Single; virtual;
    function IsStoredCustom: Boolean;
    procedure SetHeight(Value: Single); virtual;
    procedure SetLeft(Value: Single); virtual;
    procedure SetScreenPixelLeft(Value: Integer); virtual;
    procedure SetScreenPixelTop(Value: Integer); virtual;
    procedure SetScreenPixelWidth(Value: Integer); virtual;
    procedure SetScreenPixelHeight(Value: Integer); virtual;
    procedure SetTop(Value: Single); virtual;
  ...
  public
    ...
    function GetPropValue(const aPropName: string; var aValue): Boolean; virtual;
    function IsValidPropName(const aPropName: string): Boolean; virtual;

    {used to convert to 3.0}
    procedure Convert(aVersionNo: Integer); virtual;
    function SetPropValue(const aPropName: string; var aValue): Boolean; virtual;
    ...
    property Height: Single read GetHeight write SetHeight stored IsStoredCustom;
    property Left: Single read GetLeft write SetLeft stored IsStoredCustom;
    property LookupPipeline: Boolean read FLookupPipeline write FLookupPipeline;
    property mmLeft: Longint read GetmmLeft;
    property mmHeight: Longint read GetmmHeight;
    property mmTop: Longint read GetmmTop;
    property mmWidth: Longint read GetmmWidth;
    ...
    property Top: Single read GetTop write SetTop stored IsStoredCustom;
    property Units: TppUnitType read FUnits write SetUnits;
    property Visible: Boolean read FVisible write SetVisible default True;
    property Width: Single read GetWidth write SetWidth stored IsStoredCustom;
...
begin
  ...
  if Assigned(FOnDrawCommandClick) or IsActiveNotify(ciPrintableDrawCommandClick) or ((lDrawCommand is TppDrawText) and (TppDrawText(lDrawCommand).HyperLink <> '')) then
  begin
    lDrawCommand.Clickable := True;//Assigned(FOnDrawCommandClick) or IsActiveNotify(ciPrintableDrawCommandClick) or ((lDrawCommand is TppDrawText) and (TppDrawText(lDrawCommand).HyperLink <> ''));
...
{@TppPrintable.AfterDesignerCreate
 ...
 Cre  ate. (For example, the TppTeeChart component overrides this procedure and
...
begin
  Result := Rect( Trunc(ppFromMMThousandths(mmLeft,  utScreenPixels, pprtHorizontal, nil)),
                    Trunc(ppFromMMThousandths(mmTop,  utScreenPixels, pprtVertical, nil)),
                    Trunc(ppFromMMThousandths(mmLeft+mmWidth,  utScreenPixels, pprtHorizontal, nil)),
                    Trunc(ppFromMMThousandths(mmTop+mmHeight, utScreenPixels, pprtVertical, nil)) );
end;
...
begin
  Result := Rect(0, 0, Trunc(ppFromMMThousandths(mmWidth,  utScreenPixels, pprtHorizontal, nil)),
                       Trunc(ppFromMMThousandths(mmHeight, utScreenPixels, pprtVertical, nil)) );
end;
...
begin
  ...
  else // assume a move, possible from one band to another band with same relative top position
    BeforePropertyChange('MoveBounds') ;
...
begin
  ...
  Result := Trunc(ppFromMMThousandths(mmLeft, utScreenPixels, pprtHorizontal, nil));
...
begin
  ...
  Result := Trunc(ppFromMMThousandths(mmTop, utScreenPixels, pprtVertical, nil));
...
begin
  ...
  Result := Trunc(ppFromMMThousandths(mmWidth, utScreenPixels, pprtHorizontal, nil));
...
begin
  ...
  Result := Trunc(ppFromMMThousandths(mmHeight, utScreenPixels, pprtVertical, nil));
...
function TppPrintable.GetmmHeight: Longint;
begin
  Result := FHeight;
end;

function TppPrintable.GetmmLeft: Longint;
begin
  Result := FLeft;
end;

function TppPrintable.GetmmTop: Longint;
begin
  Result := FTop;
end;

function TppPrintable.GetmmWidth: Longint;
begin
  Result := FWidth;
end;

function TppPrintable.GetPropValue(const aPropName: string; var aValue): Boolean;
begin
  Result := TraRTTI.GetPropValue(Self, aPropName, aValue);
end;

function TppPrintable.SetPropValue(const aPropName: string; var aValue): Boolean;
var
  lRTTIClass: TraRTTIClass;
begin

  lRTTIClass := TraRTTIClassRegistry.GetRTTIClass(ClassType);

  if lRTTIClass.IsValidPropName(Self, aPropName) then
    Result := lRTTIClass.SetPropValue(Self, aPropName, aValue)
  else
    Result := False;

end;

function TppPrintable.IsStoredCustom: Boolean;
begin
...
function TppPrintable.IsValidPropName(const aPropName: string): Boolean;
begin
  Result := TraRTTI.IsValidPropName(Self, aPropName);
end;
...
begin
  ...
  if (CompareText(aPropName, 'Alignment') = 0) then
    ...
    IntPtr(aValue) := IntPtr(TppPrintable(aObject).DataPipeline)
    ...
    IntPtr(aValue) := IntPtr(TppPrintable(aObject).Font)
