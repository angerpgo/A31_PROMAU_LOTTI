...
unit ZZLIBRERIE_PROMAU;

interface

uses
  System.variants, System.SysUtils, System.math, System.Inifiles, System.Json, windows, System.Classes, Menus, Vcl.Controls, Vcl.Graphics, Vcl.Dialogs,  ShellApi, Forms, idGlobalProtocols, DB, MyAccess, query_go,
  raizeedit_go, rztabs, rzdbchk, DMARC, ZZCERCA_PREZZO;

type
  tlotto_promau = record
    lotto: string;
    tipo_lotto: string;
    tms_codice: string;
    progressivo: string;
...
function promau_qrcode_lottoToJSON(lotto: string): TJSONObject;
...
function promau_qrcode_lottoToJSON(lotto: string): TJSONObject;
var
  lot_json: TJsonObject;
  lot, a31eti: tmyquery_go;

begin
  lot_json := TJSONObject.create;

  try
    a31eti := tmyquery_go.create(nil);
    a31eti.connection := arc.arcdit;
    a31eti.sql.add('select a.* ');
    a31eti.sql.add('from a31etichette a ');
    a31eti.sql.add('inner join lot on lot.lotto=a.lot_codice ');
    a31eti.sql.add('where ');
    a31eti.sql.add('lot.lotto=:lotto');
    a31eti.parambyname('lotto').asstring := lotto;
    a31eti.open;

    lot := tmyquery_go.create(nil);
    lot.connection := arc.arcdit;
    lot.sql.add('select lot.*, art.descrizione1 art_descrizione1, art.tcm_codice ');
    lot.sql.add('from lot ');
    lot.sql.add('inner join art on art.codice=lot.art_codice ');
    lot.sql.add('where ');
    lot.sql.add('lot.lotto=:lotto ');
    lot.parambyname('lotto').asstring := lotto;
    lot.open;

    lot_json.AddPair(TJSONPair.Create('lotto', lot.fieldbyname('lotto').asstring));
    lot_json.AddPair(TJSONPair.Create('fornitore', copy(lot.fieldbyname('lotto').asstring, 3, 4)));
    lot_json.AddPair(TJSONPair.Create('codice articolo', lot.fieldbyname('art_codice').asstring));
    lot_json.AddPair(TJSONPair.Create('descrizione articolo', lot.fieldbyname('art_descrizione1').asstring));
    lot_json.AddPair(TJSONPair.Create('quantita', TJSONNumber.Create(a31eti.fieldbyname('quantita').asinteger)));
    lot_json.AddPair(TJSONPair.Create('data lotto', formatdatetime( 'YYMMDD', lot.fieldbyname('data_ora_creazione').asdatetime)));
    lot_json.AddPair(TJSONPair.Create('categoria articolo', lot.fieldbyname('tcm_codice').asstring));

    if lot.fieldbyname('a31_qrcode').asstring = '' then
    begin
      lot.edit;
      lot.fieldbyname('a31_qrcode').asstring := lot_json.ToJSON;
      lot.post;
    end;
